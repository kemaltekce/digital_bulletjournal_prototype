Vue.component('collection', {
  ...
  template: `
    <div class="collection">
      <div class="section-color" :class="notes[0].style === 'heading' ? 'section-color-heading' : ''"></div>
      <bullet
        v-for="note in notes" :note="note" :key="note.id" :ref="'bullet-' + note.id"
        :styles="styles"
        v-on:edit-bullet="editBullet"
        v-on:change-style="changeStyle"
  ...
  methods: {
    ...
    addBullet(note) {
      ...
      if (window.getSelection()['anchorOffset'] === 0 && note.text.length !== 0) {
    ...
    removeBullet(args) {
      ...
      var deleteIndex = (note) => note.id === deleteID
      this.notes.splice(this.notes.findIndex(deleteIndex), 1)
      this.notes.forEach(function(note) {
...
Vue.component('bullet', {
  ...
  var app = new Vue({
    ...
    methods: {
      ...
      addDefaultBullet() {
        this.page.forEach(collection => {
          var lastBullet = collection.notes[collection.notes.length - 1]
          if (lastBullet === undefined) {
            collection.notes.push({id: this.uuid(), text:"", position: 1})
