Vue.component('bullet', {
  ...
  methods: {
    edit(event) {
      ...
      this.$emit('edit-bullet', {target: this.note, new_text: src})
    },
    endEdit(event) {
...
Vue.component('section-title', {
  ...
  template: `
    <div
      ...
      @keydown.down="move"></div>
  `,
  methods: {
    edit(event) {
      var new_title = event.target.innerText
      this.$emit('edit-section-title', {target: this.title, new_text: new_title})
    },
    move(event) {
      this.$emit('move-to-first-bullet', event)
    }
  }
})
...
var app = new Vue({
  ...
  data: function() {
    return {
      sectionTitle: {id: this.uuid(), text: 'Title'},
      notes: [
  ...
  methods: {
    ...
    editText(info){
      this.$set(info.target, 'text', info.new_text)
      this.$forceUpdate()
    },
    addBullet(note) {
    ...
    moveToFirstBullet(event) {
      var frist_bullet = this.notes[0]
      this.moveTo(event, frist_bullet)
    },
    moveTo(event, bullet) {
      if (bullet) {
        this.$nextTick(() => {
